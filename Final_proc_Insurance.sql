...
--Trigger for insert on coverage table
CREATE or ALTER TRIGGER trg_Insert_Coverage
ON Coverage
AFTER INSERT
AS 
BEGIN
    ...
    -- Insert a record into the LOGINFORMATION Table
    INSERT INTO LOGINFORMATION (TableName, Operation, RecordID, ChangedBy)
    select 'Coverage','INSERT', inserted.CoverageID, SYSTEM_USER
    FROM inserted
END
GO

--Trigger for update on Coverage table
CREATE or ALTER TRIGGER trg_Update_Coverage
ON Coverage
AFTER UPDATE
AS 
BEGIN
    ...
    -- Insert a record into the LOGINFORMATION Table
    INSERT INTO LOGINFORMATION (TableName, Operation, RecordID, ChangedBy)
    select 'Coverage','UPDATE', inserted.CoverageID, SYSTEM_USER
    FROM inserted
END
GO
--Trigger for delete on policies table
CREATE or ALTER TRIGGER trg_Delete_Coverage
ON Coverage
AFTER DELETE
AS 
BEGIN
    ...
    -- Insert a record into the LOGINFORMATION Table
    INSERT INTO LOGINFORMATION (TableName, Operation, RecordID, ChangedBy)
    select 'Coverage','DELETE', deleted.CoverageID, SYSTEM_USER
    FROM deleted
END
GO
...
END
GO
        --Trigger for update on Payments table
...
END
GO
        --Trigger for delete on Payments table
...
GO
    ...
    --B. Triggers for Agents Table
        --Trigger for insert on Agents table
CREATE or ALTER TRIGGER trg_Insert_Agents
ON Agents
AFTER INSERT
AS 
BEGIN
    ...
    -- Insert a record into the LOGINFORMATION Table
    INSERT INTO LOGINFORMATION (TableName, Operation, RecordID, ChangedBy)
    select 'Agents','INSERT', inserted.AgentID, SYSTEM_USER
    FROM inserted
END
GO
        --Trigger for update on Agents table
CREATE or ALTER TRIGGER trg_Update_Agents
ON Agents
AFTER UPDATE
AS 
BEGIN
    ...
    -- Insert a record into the LOGINFORMATION Table
    INSERT INTO LOGINFORMATION (TableName, Operation, RecordID, ChangedBy)
    select 'Agents','UPDATE', inserted.AgentID, SYSTEM_USER
    FROM inserted
END
GO
        --Trigger for delete on Agents table
CREATE or ALTER TRIGGER trg_Delete_Agents
ON Agents
AFTER DELETE
AS 
BEGIN
    ...
    -- Insert a record into the LOGINFORMATION Table
    INSERT INTO LOGINFORMATION (TableName, Operation, RecordID, ChangedBy)
    select 'Agents','DELETE', deleted.AgentID, SYSTEM_USER
    FROM deleted
END
GO
